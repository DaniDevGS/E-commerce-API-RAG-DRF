...
class InventarioProductos(APIView):
    def get(self, request):
        nombre = self.request.query_params.get("nombre") # type: ignore
        subcategoria = self.request.query_params.get("subcategoria") # type: ignore
        precio = self.request.query_params.get("precio") # type: ignore
...
class InventarioProductosDetail(APIView):
    def get(self, request, pk=None):
        try:
            ...
            precio_usd = Decimal(str(data.get('precio', 0))) # type: ignore
            if cambio_bolivar is not None:
                cambio_bolivar = Decimal(str(cambio_bolivar))
                conversion = precio_usd * cambio_bolivar
                data['precio_bs'] = "{:.2f}".format(conversion) # type: ignore
            else:
                data['precio_bs'] = None # type: ignore
